/**
 * This code implements TimeIt function to measure the time taken for the point cloud methods (distance and centroid).
 * **/
...
// Prevents dead code elimination by making the compiler treat 'value'
// as if it may be used outside the current scope.
template <typename T>
...
int main()
{
    ...
    auto aos_sum_distances_time = timeIt([&]()
                                         { return sumDistancesTo(aosCloud, Point{0.0, 0.0, 0.0}); });

    auto soa_sum_distances_time = timeIt([&]()
                                         { return sumDistancesTo(soaCloud, Point{0.0, 0.0, 0.0}); });

    cout << "soa_centroid_time: " << soa_centroid_time << endl;
    cout << "aos_centroid_time: " << aos_centoid_time << endl;
    cout << "aos_sum_distances_time: " << aos_sum_distances_time << endl;
    cout << "soa_sum_distances_time: " << soa_sum_distances_time << endl;

    auto SOA_Cloud_sumOnlyXTime = timeIt([&]()
                                         { return sumOnlyX(soaCloud); });

    auto aosCloud_sumOnlyXTime = timeIt([&]()
                                        { return sumOnlyX(aosCloud); });
    cout << "SOA Cloud: Sum only X Time:" << SOA_Cloud_sumOnlyXTime << endl;
    cout << "AOS Cloud: Sum only X Time:" << aosCloud_sumOnlyXTime << endl;
}